...
module.exports = async (client, interaction) => {
    ...
    if (command.permissionsNeeded && !interaction.member.permissions.has(command.permissionsNeeded))
        return interaction.reply({ content: trans('NO_PERMISSION', lang, interaction.user), ephemeral: true });

    if (command.botPermsNeeded && !interaction.guild.me.permissions.has(command.botPermsNeeded))
        return interaction.reply({ content: trans('NEED_PERMS', lang, command.botPermsNeeded.join(', ')), ephemeral: true });
